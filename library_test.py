...
async def main() -> None:
    ...
    class_name, url = await VodafoneStationCommonApi.get_device_type(
        args.router,
        session,
    )

    print(f"Device type: {class_name} ({url})")

    print("-" * 20)
    api_class: type[VodafoneStationCommonApi] = class_registry[class_name]
