...
class VodafoneStationHomewareApi(VodafoneStationCommonApi):
    ...
    async def _get_csrf_token(self) -> str:
        ...
        reply: ClientResponse = await self._request_page_result(
            HTTPMethod.GET,
    ...
    async def login(self, force_logout: bool = False) -> bool:
        ...
        auth1_reply: ClientResponse = await self._request_page_result(
            HTTPMethod.POST,
            "authenticate",
            payload={
    ...
    async def get_devices_data(self) -> dict[str, VodafoneStationDevice]:
        ...
        reply = await self._request_page_result(
            HTTPMethod.GET, "modals/overview.lp", query={"status": "WifiInfo"}
        )
        if reply.content_type == "application/json":
            ...
            wifi_reply = await self._request_page_result(
                HTTPMethod.GET, "modals/overview.lp", query={"status": "wifiInfo"}
            )
            eth_reply = await self._request_page_result(
                HTTPMethod.GET, "modals/overview.lp", query={"status": "networkInfo"}
            )
    ...
    async def get_sensor_data(self) -> dict[str, Any]:
        ...
        sysinfo_reply = await self._request_page_result(
            HTTPMethod.GET,
            "modals/status-support/status.lp",
            query={"status": "systemInfo"},
        )
        interfaces_reply = await self._request_page_result(
            HTTPMethod.GET,
            "modals/status-support/restart.lp",
            query={"getInterfaceValues": "true"},
        )
    ...
    async def _get_wifi_settings(self) -> dict[str, Any]:
        """Parse current WiFi settings from the general settings page HTML."""
        reply = await self._request_page_result(
            HTTPMethod.GET, "modals/wifi/general.lp", query={}
        )
